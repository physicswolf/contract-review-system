# 合同审查配置中心技术设计

**版本**: v4.0  
**日期**: 2026-06-25  
**基于**: 配置中心技术设计 v3.0 + AI智契 PRD 补充

---

# 一、概要

## 1.1 背景

合同审查系统需要灵活的审查规则管理能力。配置中心提供可视化工作台，支持审查点（含维度、风险点判断标准）的维护，以及合同类型与审查点的关联配置。

相比 v3.0，本版本基于 AI智契 PRD 补充了以下能力：
- 合同类型新增**识别关键词**字段，供大模型自动匹配合同类型
- 审查点与合同类型**完全解耦**，关联关系支持按合同类型独立启停
- 合同类型支持**多立场**（甲方/乙方），每个立场独立配置审查策略

## 1.2 功能目标

**Tab 1 - 审查点维护**

- 维度管理：新增/编辑/删除维度，删除前校验是否有审查点引用
- 审查点 CRUD：按维度分组展示，支持关键词搜索，支持启用/停用
- 审查点详情：名称、维度、描述、审查说明、风险点列表（高/低/无风险判断标准）、审查示例、默认结论

**Tab 2 - 合同类型配置**

- 合同类型 CRUD：编码、名称、立场、描述、**识别关键词**、启用状态
- 审查点关联：为合同类型绑定审查点（按维度分组多选），支持单独启停
- 确认保存：全量替换该合同类型的审查点关联

## 1.3 非功能目标

- 接口响应 P95 < 500ms，直接读 DB，无需缓存层
- 配置变更立即生效

## 1.4 与 v3.0 主要差异

| 变化点 | v3.0 | v4.0 |
|--------|------|------|
| 合同类型字段 | 无关键词 | 新增 `keywords` JSON 字段 |
| 合同类型立场 | name+stance 唯一键 | 合同类型与立场解耦，立场在关联表体现 |
| 审查点状态 | 无独立状态 | 新增 `enabled` 字段 |
| 关联表 | 仅绑定关系 | 新增 `enabled` 字段支持按类型启停 |
| 维度状态 | 无启停 | 新增 `enabled` 字段，支持维度级启停 |

---

# 二、整体方案设计

## 2.1 系统架构

```
┌─────────────────────────────────────────┐
│           前端 SPA（配置中心）            │
│  Tab1: 审查点维护（维度 + 审查点管理）    │
│  Tab2: 合同类型配置（类型 + 审查点关联）  │
└──────────────────┬──────────────────────┘
                   │ REST API / JSON
┌──────────────────▼──────────────────────┐
│              后端服务                    │
│  DimensionService                       │
│  AuditPointService                      │
│  ContractTypeService                    │
└──────────────────┬──────────────────────┘
                   │
┌──────────────────▼──────────────────────┐
│               MySQL                     │
│  dimension / audit_point                │
│  contract_type / contract_type_audit_point │
└──────────────────┬──────────────────────┘
                   │ 直接查询（按合同类型+立场）
┌──────────────────▼──────────────────────┐
│              AI 审查引擎                 │
│  1. 依据关键词+描述自动匹配合同类型       │
│  2. 加载该类型启用的审查点规则            │
└─────────────────────────────────────────┘
```

## 2.2 模块职责

| 模块 | 职责 |
|------|------|
| DimensionService | 维度 CRUD，删除前校验引用 |
| AuditPointService | 审查点 CRUD，JSON 字段整体读写，启停管理 |
| ContractTypeService | 合同类型 CRUD + 识别关键词管理 + 审查点关联全量替换（事务） |

## 2.3 数据流

**配置写入流**：管理员操作 → REST API → Service 层校验 → MySQL 持久化  
**AI 识别流**：AI 引擎传入合同文本 → 匹配 `contract_type`（name/description/keywords）→ 确定合同类型+立场  
**AI 读取流**：合同类型 id → 查关联表（enabled=1）→ 批量读取审查点详情（含 JSON 字段）

---

# 三、数据库设计

> 原则：MySQL，四张表，JSON 列存储复合结构，无独立查询场景不拆表。

## 3.1 ER 图

```
dimension
  id (PK)
  name / description / sort_order
    │ 1:N
    ▼
audit_point
  id (PK)
  dim_id (FK)
  name / description / instruction
  risk_points (JSON) / examples (JSON) / default_result (JSON)
  enabled / deleted_at
    │ N:M
    ▼
contract_type_audit_point
  contract_type_id (FK)
  audit_point_id (FK)
  enabled / sort_order
    ▲ 1:N
    │
contract_type
  id (PK)
  code / name / stance / description
  keywords (JSON)
  enabled / deleted_at
```

## 3.2 维度表 `dimension`

```sql
CREATE TABLE `dimension` (
  `id`          BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `name`        VARCHAR(100)    NOT NULL COMMENT '维度名称',
  `description` TEXT                     COMMENT '维度说明',
  `sort_order`  INT             NOT NULL DEFAULT 0,
  `enabled`     TINYINT         NOT NULL DEFAULT 1 COMMENT '1启用 0停用',
  `created_at`  DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at`  DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_name` (`name`),
  KEY `idx_enabled` (`enabled`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='审查维度';
```

## 3.3 审查点表 `audit_point`

```sql
CREATE TABLE `audit_point` (
  `id`             BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `dim_id`         BIGINT UNSIGNED NOT NULL                  COMMENT '所属维度 ID',
  `name`           VARCHAR(200)    NOT NULL                  COMMENT '审查点名称',
  `description`    VARCHAR(500)                              COMMENT '描述',
  `instruction`    TEXT                                      COMMENT '审查说明',
  `risk_points`    JSON                                      COMMENT '风险点列表',
  `examples`       JSON                                      COMMENT '审查示例列表',
  `default_result` JSON                                      COMMENT '默认结论',
  `enabled`        TINYINT         NOT NULL DEFAULT 1        COMMENT '1启用 0停用',
  `sort_order`     INT             NOT NULL DEFAULT 0,
  `created_at`     DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at`     DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `deleted_at`     DATETIME                 DEFAULT NULL,
  PRIMARY KEY (`id`),
  KEY `idx_dim` (`dim_id`, `deleted_at`, `enabled`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='审查点';
```

**JSON 字段结构**：

`risk_points`：
```json
[
  {
    "name": "主体信息缺失",
    "highStd": "任一方名称、地址、证件号缺失",
    "lowStd": "存在但表述不规范",
    "noneStd": "双方主体信息完整且清晰"
  }
]
```

`examples`：
```json
[
  {
    "original": "甲方：（盖章处）",
    "level": "高风险",
    "analysis": "仅留盖章处，未填写具体主体信息",
    "suggestion": "补充完整的公司名称、法定地址"
  }
]
```

`default_result`：
```json
{
  "level": "高风险",
  "analysis": "合同主体信息不完整，存在较大法律风险",
  "suggestion": "要求对方补充完整主体信息"
}
```

## 3.4 合同类型表 `contract_type`

相比 v3.0 新增 `keywords` 字段（JSON 数组），供大模型自动匹配合同类型；移除 `code` 字段的唯一约束（改为 name+stance 唯一）。

```sql
CREATE TABLE `contract_type` (
  `id`          BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `code`        VARCHAR(50)              COMMENT '类型编码，如 CT-001',
  `name`        VARCHAR(100)    NOT NULL COMMENT '合同类型名称',
  `stance`      VARCHAR(50)     NOT NULL COMMENT '立场，如甲方/乙方',
  `description` TEXT                     COMMENT '合同描述',
  `keywords`    JSON                     COMMENT '识别关键词列表，如["采购","供货","设备"]',
  `enabled`     TINYINT         NOT NULL DEFAULT 1 COMMENT '1启用 0停用',
  `created_at`  DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at`  DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `deleted_at`  DATETIME                 DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_name_stance` (`name`, `stance`),
  KEY `idx_enabled` (`enabled`, `deleted_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='合同类型';
```

`keywords` 示例：
```json
["采购", "供货", "设备", "价款"]
```

## 3.5 合同类型-审查点关联表 `contract_type_audit_point`

相比 v3.0 新增 `enabled` 字段，支持某合同类型下独立启停某审查点，无需修改全局审查点。

```sql
CREATE TABLE `contract_type_audit_point` (
  `id`               BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `contract_type_id` BIGINT UNSIGNED NOT NULL,
  `audit_point_id`   BIGINT UNSIGNED NOT NULL,
  `enabled`          TINYINT         NOT NULL DEFAULT 1 COMMENT '该合同类型下是否启用此审查点',
  `sort_order`       INT             NOT NULL DEFAULT 0,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_ct_ap` (`contract_type_id`, `audit_point_id`),
  KEY `idx_ct` (`contract_type_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='合同类型-审查点关联';
```

---

# 四、API 接口设计

**基础路径**：`/api/v1/config`  
**通用响应格式**：`{ "code": 0, "msg": "ok", "data": ... }`

## 4.1 通用错误码

| code | HTTP 状态码 | 说明 |
|------|------------|------|
| 0 | 200 | 成功 |
| 40001 | 400 | 参数校验失败 |
| 40401 | 404 | 资源不存在 |
| 40901 | 409 | 名称/编码重复，或有引用无法删除 |
| 50001 | 500 | 内部服务错误 |

---

## 4.2 维度接口

### GET /dimensions — 获取维度列表

**Query 参数**：

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| enabled | number | 否 | 1/0 过滤启用状态，不传则返回全部 |

**响应**：
```json
{
  "code": 0,
  "data": [
    { "id": 1, "name": "基础核查", "description": "主体信息、签约授权等", "sortOrder": 0, "enabled": 1, "updatedAt": "2026-06-25T10:00:00+08:00" },
    { "id": 2, "name": "法务风险", "description": "权利义务、违约责任等", "sortOrder": 1, "enabled": 1, "updatedAt": "2026-06-25T10:00:00+08:00" }
  ]
}
```

### POST /dimensions — 创建维度

**请求**：

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| name | string | 是 | 维度名称，唯一，≤100字 |
| description | string | 否 | 维度说明 |
| sortOrder | number | 否 | 排序，默认 0 |
| enabled | number | 否 | 默认 1 |

**响应**：`{ "code": 0, "data": { "id": 1 } }`  
**错误**：名称重复 → `{ "code": 40901, "msg": "维度名称已存在" }`

### PUT /dimensions/:id — 更新维度

**请求**：

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| name | string | 否 | 维度名称，≤100字 |
| description | string | 否 | 维度说明 |
| enabled | number | 否 | 1启用 0停用 |

**响应**：`{ "code": 0 }`  
**错误**：名称重复 → `{ "code": 40901, "msg": "维度名称已存在" }`

### DELETE /dimensions/:id — 删除维度

**业务规则**：维度下存在审查点（未软删除）则拒绝删除，需先删除或迁移该维度下的所有审查点。

**响应**：
- 成功：`{ "code": 0 }`
- 有引用：`{ "code": 40901, "msg": "该维度下存在 N 个审查点，请先删除或迁移后再操作" }`

---

## 4.3 审查点接口

### GET /audit-points — 查询审查点列表

**Query 参数**：

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| dimId | number | 否 | 按维度过滤 |
| keyword | string | 否 | 按名称模糊搜索 |
| enabled | number | 否 | 1/0 过滤状态 |

**响应**（不含 JSON 大字段，`riskPoints` 仅返回名称列表用于列表展示）：
```json
{
  "code": 0,
  "data": [
    {
      "id": 1,
      "name": "主体信息完整性",
      "description": "采购合同-甲方",
      "dimId": 1,
      "dimName": "基础核查",
      "riskPoints": ["主体信息缺失", "授权不足"],
      "enabled": 1,
      "updatedAt": "2026-06-25T10:00:00+08:00"
    }
  ]
}
```

### POST /audit-points — 创建审查点

**请求**：

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| dimId | number | 是 | 所属维度 ID |
| name | string | 是 | 审查点名称，≤200字 |
| description | string | 否 | 描述，≤500字 |
| instruction | string | 否 | 审查说明 |
| riskPoints | array | 否 | 风险点列表 |
| riskPoints[].name | string | 是 | 风险点名称 |
| riskPoints[].highStd | string | 否 | 高风险判断标准 |
| riskPoints[].lowStd | string | 否 | 低风险判断标准 |
| riskPoints[].noneStd | string | 否 | 无风险判断标准 |
| examples | array | 否 | 审查示例列表 |
| examples[].original | string | 是 | 原文 |
| examples[].level | string | 是 | 风险等级：高风险/低风险/无风险 |
| examples[].analysis | string | 否 | 分析说明 |
| examples[].suggestion | string | 否 | 修改建议 |
| defaultResult | object | 否 | 默认结论 |
| defaultResult.level | string | 是 | 风险等级 |
| defaultResult.analysis | string | 否 | 分析 |
| defaultResult.suggestion | string | 否 | 建议 |

**响应**：`{ "code": 0, "data": { "id": 1 } }`

### GET /audit-points/:id — 获取审查点详情

**响应**（含全部 JSON 字段）：
```json
{
  "code": 0,
  "data": {
    "id": 1,
    "dimId": 1,
    "dimName": "基础核查",
    "name": "主体信息完整性",
    "description": "采购合同-甲方立场",
    "instruction": "核查合同双方主体信息是否完整",
    "riskPoints": [...],
    "examples": [...],
    "defaultResult": { "level": "高风险", "analysis": "...", "suggestion": "..." },
    "enabled": 1,
    "updatedAt": "2026-06-25T10:00:00+08:00"
  }
}
```

### PUT /audit-points/:id — 更新审查点

请求结构同创建，所有字段可选，JSON 字段整体覆盖写入。**响应**：`{ "code": 0 }`

### PATCH /audit-points/:id/enabled — 启停审查点

**请求**：`{ "enabled": 0 }`  
**响应**：`{ "code": 0 }`

### DELETE /audit-points/:id — 软删除审查点

写 `deleted_at = NOW()`。**响应**：`{ "code": 0 }`

---

## 4.4 合同类型接口

### GET /contract-types — 查询合同类型列表

**Query 参数**：

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| enabled | number | 否 | 1/0 过滤启用状态 |
| keyword | string | 否 | 按名称/编码/关键词搜索 |

**响应**：
```json
{
  "code": 0,
  "data": [
    {
      "id": 1,
      "code": "CT-001",
      "name": "采购合同",
      "stance": "甲方",
      "description": "以购买货物、设备或软件产品为主要目的",
      "enabled": 1,
      "linkedPointCount": 12
    }
  ]
}
```

### POST /contract-types — 创建合同类型

**请求**：

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| code | string | 否 | 类型编码，如 CT-001，≤50字 |
| name | string | 是 | 名称，≤100字 |
| stance | string | 是 | 立场，如甲方/乙方，≤50字 |
| description | string | 否 | 描述 |
| keywords | array | 否 | 识别关键词列表，字符串数组 |
| enabled | number | 否 | 默认 1 |

**响应**：`{ "code": 0, "data": { "id": 1 } }`  
**错误**：名称+立场重复 → `{ "code": 40901, "msg": "该合同类型-立场已存在" }`

### PUT /contract-types/:id — 更新合同类型

请求字段同创建，均可选。**响应**：`{ "code": 0 }`

### DELETE /contract-types/:id — 删除合同类型

软删除 `contract_type`，同时物理删除 `contract_type_audit_point` 中该合同类型的关联（事务执行）。  
**响应**：`{ "code": 0 }`

### GET /contract-types/:id — 获取合同类型详情

**响应**（含已关联审查点 ID 列表，用于编辑页回显）：
```json
{
  "code": 0,
  "data": {
    "id": 1,
    "code": "CT-001",
    "name": "采购合同",
    "stance": "甲方",
    "description": "以购买货物、设备或软件产品为主要目的",
    "keywords": ["采购", "供货", "设备", "价款"],
    "enabled": 1,
    "linkedAuditPoints": [
      { "auditPointId": 1, "enabled": 1 },
      { "auditPointId": 2, "enabled": 0 }
    ]
  }
}
```

> `linkedAuditPoints` 来自 `contract_type_audit_point`，前端据此回显勾选状态及类型级启停。

### PUT /contract-types/:id/audit-points — 保存审查点关联（全量替换）

**请求**：

| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| auditPointIds | number[] | 是 | 审查点 ID 列表，传空数组表示清空 |

```json
{ "auditPointIds": [1, 2, 3] }
```

**执行逻辑**（事务）：
1. `DELETE FROM contract_type_audit_point WHERE contract_type_id = ?`
2. 批量 `INSERT INTO contract_type_audit_point (contract_type_id, audit_point_id, sort_order, enabled) VALUES ...`

**响应**：`{ "code": 0 }`

**前端审查点选择交互（创建/编辑页）**：

页面审查点选择区支持按维度筛选，交互流程如下：

1. 进入页面时调用 `GET /dimensions?enabled=1` 渲染维度 Tab（全部 / 基础核查 / 法务风险 / …）
2. 切换维度 Tab 时携带 `dimId` 参数调用 `GET /audit-points?enabled=1&dimId={id}` 拉取该维度下的审查点列表
3. 编辑场景额外调用 `GET /contract-types/:id` 获取已关联的 `linkedAuditPoints`，在列表中回显勾选状态
4. 用户勾选/取消后本地维护已选 ID 集合，点击保存时统一调用 `PUT /contract-types/:id/audit-points`

```
维度筛选 Tab：[全部] [基础核查] [法务风险] [履约检查] ...
                         ↓ dimId=1
审查点列表（多选）：
  ☑ 主体信息完整性    基础核查
  ☐ 合同标的与范围    基础核查
  ☑ 附件与签章完整性  基础核查
```

> `GET /audit-points` 已有 `dimId` 过滤参数，无需新增接口。

### PATCH /contract-types/:id/audit-points/:apId/enabled — 启停某合同类型下的审查点

**请求**：`{ "enabled": 0 }`  
**响应**：`{ "code": 0 }`

> 此接口仅更新关联表的 `enabled` 字段，不影响审查点全局状态。

---

## 4.5 AI 引擎专用接口

供 AI 审查引擎内部调用，不对前端暴露。

### GET /internal/contract-types/match — 匹配合同类型

**Query 参数**：

| 参数 | 类型 | 必填 | 说明 |
|------|------|------|------|
| name | string | 是 | 合同类型名称 |
| stance | string | 是 | 立场 |

**响应**：
```json
{
  "code": 0,
  "data": {
    "id": 1,
    "code": "CT-001",
    "name": "采购合同",
    "stance": "甲方",
    "keywords": ["采购", "供货", "设备", "价款"]
  }
}
```

### GET /internal/contract-types/:id/audit-points — 加载审查点规则

**响应**（仅返回已启用审查点，含 JSON 字段）：
```json
{
  "code": 0,
  "data": [
    {
      "id": 1,
      "name": "主体信息完整性",
      "instruction": "核查合同双方主体信息是否完整",
      "riskPoints": [...],
      "examples": [...],
      "defaultResult": { "level": "高风险", "analysis": "...", "suggestion": "..." }
    }
  ]
}
```

---

# 五、关键业务逻辑

## 5.1 维度删除保护

```
DELETE /dimensions/:id
  → SELECT COUNT(*) AS cnt FROM audit_point WHERE dim_id = ? AND deleted_at IS NULL
  → cnt > 0: 返回 40901，msg = "该维度下存在 {cnt} 个审查点，请先删除或迁移后再操作"
  → cnt = 0: DELETE FROM dimension WHERE id = ?
```

## 5.2 合同类型-审查点关联全量替换

全量替换策略保证幂等，事务内先清后写：

```sql
BEGIN;
DELETE FROM contract_type_audit_point WHERE contract_type_id = ?;
INSERT INTO contract_type_audit_point (contract_type_id, audit_point_id, sort_order, enabled)
VALUES (?, ?, ?, 1), ...;
COMMIT;
```

## 5.3 AI 引擎读取链路

```sql
-- 1. 查合同类型（支持 name+stance 精确匹配）
SELECT id, keywords FROM contract_type
WHERE name = ? AND stance = ? AND enabled = 1 AND deleted_at IS NULL;

-- 2. 查关联的已启用审查点（双层启用：全局 + 该类型下）
SELECT ap.id, ap.name, ap.instruction, ap.risk_points, ap.examples, ap.default_result
FROM contract_type_audit_point ctap
JOIN audit_point ap ON ap.id = ctap.audit_point_id
WHERE ctap.contract_type_id = ?
  AND ctap.enabled = 1
  AND ap.enabled = 1
  AND ap.deleted_at IS NULL
ORDER BY ctap.sort_order;
```

## 5.4 审查点启停逻辑

- **全局停用**（`audit_point.enabled=0`）：该审查点在所有合同类型下均不生效
- **类型级停用**（`contract_type_audit_point.enabled=0`）：仅在该合同类型下不生效，其他合同类型不受影响
- AI 引擎读取时两个条件均需满足（`AND` 关系）

---

# 六、排期估算

| 模块 | 任务 | 预估工时 |
|------|------|---------|
| 后端 | 维度 CRUD | 0.5 人日 |
| 后端 | 审查点 CRUD（含 JSON 字段 + 启停） | 1 人日 |
| 后端 | 合同类型 CRUD（含 keywords 字段） | 0.5 人日 |
| 后端 | 合同类型-审查点关联 + 类型级启停 | 1 人日 |
| 后端 | AI 引擎内部接口 | 0.5 人日 |
| 前端 | Tab1 审查点维护 | 2 人日 |
| 前端 | Tab2 合同类型配置 | 2 人日 |
| 测试 | 接口联调 + 功能验收 | 1 人日 |
| **合计** | | **8.5 人日** |

---
